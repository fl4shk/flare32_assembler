...
ENC_GROUP(0) ARGS(ra_rb) VARNAME(Ldr) VALUE("ldr") \
\
/* Store rA to address in rB */ \
ENC_GROUP(0) ARGS(ra_rb) VARNAME(Str) VALUE("str")



#define LIST_OF_INSTR_G0_FLAGS__COLLECTION_0(ENC_GROUP, ARGS, VARNAME, \
	VALUE) \
/* Push flags as 8-bit value onto the stack (high 4 bits of pushed */ \
/* value are set to zero before the push) */ \
ENC_GROUP(0) ARGS(flags) VARNAME(Push) VALUE("push") \
\
/* Pop 8-bit value from the stack and store low 4 bits to flags */ \
ENC_GROUP(0) ARGS(flags) VARNAME(Pop) VALUE("pop")


#define LIST_OF_INSTR_G0_RA_FLAGS__COLLECTION_0(ENC_GROUP, ARGS, VARNAME, \
	VALUE) \
/* Clear rA, then CoPY FRom Flags to rA */ \
ENC_GROUP(0) ARGS(ra_flags) VARNAME(Cpy) VALUE("cpy")

#define LIST_OF_INSTR_G0_FLAGS_RA__COLLECTION_0(ENC_GROUP, ARGS, VARNAME, \
	VALUE) \
/* Copy to flags from rA */ \
/* This instruction can affect N, V, Z, and C flags. */ \
ENC_GROUP(0) ARGS(flags_ra) VARNAME(Cpy) VALUE("cpy")


#define LIST_OF_INSTR_G0_NO_ARGS__COLLECTION_0(ENC_GROUP, ARGS, VARNAME, \
	VALUE) \
/* ENable Interrupts */ \
ENC_GROUP(0) ARGS(no_args) VARNAME(Eni) VALUE("eni") \
\
/* DIsable Interrupts */ \
ENC_GROUP(0) ARGS(no_args) VARNAME(Dii) VALUE("dii") \
\
/* Set the PC to interrupt RETurn address and enable Interrupts */ \
ENC_GROUP(0) ARGS(no_args) VARNAME(Reti) VALUE("reti") \


#define LIST_OF_INSTR_G0_IRA__COLLECTION_0(ENC_GROUP, ARGS, VARNAME, \
	VALUE) \
/* Set the PC to the Interrupt Return Address, but DON'T enable */ \
/* interrupts */ \
ENC_GROUP(0) ARGS(ira) VARNAME(Jump) VALUE("jump")
...
LIST_OF_INSTR_G0_FLAGS__COLLECTION_0(ENC_GROUP, ARGS, VARNAME, VALUE) \
LIST_OF_INSTR_G0_RA_FLAGS__COLLECTION_0(ENC_GROUP, ARGS, VARNAME, VALUE) \
LIST_OF_INSTR_G0_FLAGS_RA__COLLECTION_0(ENC_GROUP, ARGS, VARNAME, VALUE) \
LIST_OF_INSTR_G0_NO_ARGS__COLLECTION_0(ENC_GROUP, ARGS, VARNAME, VALUE) \
LIST_OF_INSTR_G0_IRA__COLLECTION_0(ENC_GROUP, ARGS, VARNAME, VALUE) \
