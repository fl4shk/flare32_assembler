...
#define LIST_OF_INSTR_G0_RA_RB__COLLECTION_0(VARNAME, VALUE) \
/* rA = rA + rB */ \
/* This instruction can affect N, V, Z, and C flags. */ \
VARNAME(Add) VALUE("add") \
\
/* rA = rA + rB + carry_flag */ \
/* Add with carry */ \
/* This instruction can affect N, V, Z, and C flags. */ \
VARNAME(Adc) VALUE("adc") \
\
/* rA = rA - rB */ \
/* This instruction can affect N, V, Z, and C flags. */ \
VARNAME(Sub) VALUE("sub") \
\
/* rA = rA + (~rB) + carry_flag */ \
/* Subtract with borrow (6502 style) */ \
/* This instruction can affect N, V, Z, and C flags. */ \
VARNAME(Sbc) VALUE("sbc") \
\
\
\
/* rA = rB - rA */ \
/* This instruction can affect N, V, Z, and C flags. */ \
VARNAME(Rsb) VALUE("rsb") \
\
/* rA = rA * rB */ \
VARNAME(Mul) VALUE("mul") \
\
/* rA = rA & rB */ \
/* This instruction can affect the N and Z flags. */ \
VARNAME(And) VALUE("and") \
\
/* rA = rA | rB */ \
/* This instruction can affect the N and Z flags. */ \
VARNAME(Or) VALUE("or") \
...
#define LIST_OF_INSTR_G0_RA_FLAGS__COLLECTION_0(VARNAME, VALUE) \
/* Clear rA, then CoPY FRom Flags to rA */ \
VARNAME(Cpy) VALUE("cpy")

#define LIST_OF_INSTR_G0_FLAGS_RA__COLLECTION_0(VARNAME, VALUE) \
/* Copy to flags from rA */ \
/* This instruction can affect N, V, Z, and C flags. */ \
VARNAME(Cpy) VALUE("cpy")


#define LIST_OF_INSTR_G0_NO_ARGS__COLLECTION_0(VARNAME, VALUE) \
/* ENable Interrupts */ \
VARNAME(Eni) VALUE("eni") \
\
/* DIsable Interrupts */ \
VARNAME(Dii) VALUE("dii") \
\
/* Set the PC to interrupt RETurn address and enable Interrupts */ \
VARNAME(Reti) VALUE("reti") \


#define LIST_OF_INSTR_G0_IRA__COLLECTION_0(VARNAME, VALUE) \
/* Set the PC to the Interrupt Return Address, but DON'T enable */ \
/* interrupts */ \
VARNAME(Jump) VALUE("jump")



#define LIST_OF_INSTR_G0_IRA_RA__COLLECTION_0(VARNAME, VALUE) \
/* Copy rA to the Interrupt Return Address */ \
VARNAME(Cpy) VALUE("cpy")

#define LIST_OF_INSTR_G0_RA_IRA__COLLECTION_0(VARNAME, VALUE) \
/* Copy the Interrupt Return Address to rA */ \
VARNAME(Cpy) VALUE("cpy")


#define LIST_OF_INSTR_G0_RA_RB__COLLECTION_1(VARNAME, VALUE) \
/* Subroutine call using (rA + rB) as destination address.  */ \
/* The return address is stored in the link register (lr). */ \
VARNAME(Callx) VALUE("callx")

#define LIST_OF_INSTR_G0_RA_PC__COLLECTION_0(VARNAME, VALUE) \
